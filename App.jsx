...
const Finances = ({ data, setData, isCommittee }) => {
  ...
  const [form, setForm] = useState({ description: "", amount: "", date: new Date().toISOString().split("T")[0], category: "", receipt: null });
...
const Faults = ({ data, setData, user, isCommittee }) => {
  const [showModal, setShowModal] = useState(false);
  const [form, setForm] = useState({ description: "", location: "", priority: "medium", files: [] });
...
const Inspections = ({ data, setData, isCommittee }) => {
  const [showModal, setShowModal] = useState(false);
  const [form, setForm] = useState({ name: "", type: "", date: "", nextDate: "", notes: "", files: [] });
...
const Protocols = ({ data, setData, isCommittee }) => {
  ...
  const [form, setForm] = useState({ title: "", date: new Date().toISOString().split("T")[0], attendees: "", content: "", files: [] });
...
const Residents = ({ data, setData, user, isCommittee }) => {
  ...
  const [form, setForm] = useState({ name: "", email: "", apartment: "", phone: "", role: "resident" });
...
// MAIN APP
// ============================================================
export default function VaadBayitApp() {
  const [user, setUser] = useState(null);
  const [activeTab, setActiveTab] = useState("home");
  const [data, setDataState] = useState(getStorage());
